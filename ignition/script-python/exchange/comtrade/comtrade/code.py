# This is a modified version of pyComtrade adapted to work with Ignition and Jython
...
class ComtradeRecord(object):
	...
	def get_timestamps(self):
		...
		return [i * t_end / (n_samples - 1) for i in range(n_samples)]
	...
	def read_ascii(self, file_path):
		...
		# Convert values
		data = [list(x) for x in zip(*data)]  # Transpose the data
		...
		for cidx in range(self.cfg_data['#A']):
			...
			# Reading channels
			values = [float(value) for value in data[cidx]]
			values = [value * self.cfg_data['A'][cidx]['a'] for value in values]
			values = [value + self.cfg_data['A'][cidx]['b'] for value in values]
	...
	def read_bin_digital(self):
		...
		for cidx in range(self.cfg_data['#D']):
			...
			# Converting values
			self.cfg_data['D'][cidx]['values'] = values
